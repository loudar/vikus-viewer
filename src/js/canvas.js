function Canvas() {
    ...
    const x = d3.scale.ordinal().rangeBands([margin.left, width + margin.left], 0.2);
    ...
    canvas.abs2relCoordinate = function (p) {
        return [(p[0] / widthOuter) * 100, ((-1 * p[1]) / widthOuter) * 100].map(function (d) {
    ...
    canvas.getView = function () {
        ...
        data.forEach(function (d) {
            ...
            if (left >= viewLeft && right <= viewRight && top >= viewTop && bottom <= viewBottom) {
                visibleItems.push(d);
            }
        });
        ...
        const unique = new Set([mostLeft?.id, mostRight?.id, mostTop?.id, mostBottom?.id]);
    ...
    canvas.setView = function (ids, duration) {
        ...
        const xs = items.map(function (d) {
            return d.x;
        });
        const ys = items.map(function (d) {
            return d.y;
        });
    ...
    canvas.makeScales = function () {
        ...
        zoomedToImageScale =
            (0.8 / (rangeBand / columns / width)) * (state.mode.type === "group" ? 1 : 0.5);
    ...
    function updateImageBorders(borderIds) {
        const enter = borderIds.filter(function (d) {
            return !imageBorders.hasOwnProperty(d);
        });
        const exit = Object.keys(imageBorders).filter(function (d) {
    ...
    function stackYLayout(data, invert) {
        ...
        const yscale = d3.scale.linear().domain(yExtent).range(yRange);
    ...
    canvas.onhashchange = function () {
        ...
        if (params.has("filter")) {
            ...
            // Crossfilter uses dim:value pairs separated by |, tags use comma
            const filter =
                filterStr.indexOf(":") > -1 ? filterStr.split("|") : filterStr.split(",");
    ...
    canvas.project = function () {
        ...
        zoomedToImageScale =
            (0.8 / (x.rangeBand() / columns / width)) * (state.mode.type === "group" ? 1 : 0.5);
    };
