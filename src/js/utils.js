...
utils.debug = false;
utils.log = function () {
    if (utils.debug) console.log.apply(console, arguments);
};
...
utils.initConfig = function (config) {
    ...
    document.styleSheets[0].insertRule(
        ".close::before { background-color: " + config.style.fontColorActive + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".close::after { background-color: " + config.style.fontColorActive + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".tag.active { color: " + config.style.fontColorActive + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".tag.active { background: " + config.style.fontBackground + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".crossfilter .keys .item.active { color: " + config.style.fontColorActive + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".crossfilter .keys .item.active { background: " + config.style.fontBackground + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".crossfilter .keys .item:hover { color: " + config.style.fontColorActive + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".crossfilter .keys .item:hover { background: " + config.style.fontBackground + "}",
        length
    );
    document.styleSheets[0].insertRule(
        ".crossfilter .keys .item { background: " + config.style.textShadow + "}",
        length
    );
    document.styleSheets[0].insertRule(
...
utils.nearest = function (x, y, best, node) {
    ...
    if (kids[bt * 2 + (1 - rl)]) best = utils.nearest(x, y, best, kids[bt * 2 + (1 - rl)]);
    if (kids[(1 - bt) * 2 + rl]) best = utils.nearest(x, y, best, kids[(1 - bt) * 2 + rl]);
    if (kids[(1 - bt) * 2 + (1 - rl)])
